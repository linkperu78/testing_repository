...
from smartlink_api.models.cambium_data_models import CambiumData

router = APIRouter()

@router.post("/add")
async def add_cambium(cambium_data: CambiumData):
...
async def add_cambium_list(list_model: List[CambiumData]):
    conn_dep = get_db_connection("smartlink")
    async with conn_dep as conn:
...
@router.get("/get_inventario/", summary="Obtener datos de equipos Cambium haciendo left join con inventario")
async def get_cambium_inventario(
    start_date: Optional[str] = Query(None, description="Fecha de inicio en formato YYYY-MM-DD"),
    end_date: Optional[str] = Query(None, description="Fecha de fin en formato YYYY-MM-DD"),
    limit: int = Query(1000, description="Número máximo de registros por página"),
    offset: int = Query(0, description="Desplazamiento para paginación"),
    ...
    async with conn_dep as conn:
        return fetch_data_with_filter_and_pagination_with_inventory(
            conn, "cambium_data", start_date, end_date, limit, offset, round_to_quarter_hour
        )