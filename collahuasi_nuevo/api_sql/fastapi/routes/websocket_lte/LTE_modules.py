# router/websocket_lte/LTE_modules.py
import re
import paramiko
import subprocess
...
def getSshData(ip_server : str, _user : str = 'admin', _pass : str = 'HCG') -> dict:
    command_ssh_list = [
    ...
    try:
        ...
        client.connect(ip_server, username = _user.strip(), password = _pass.strip(), port = 22, timeout = 10,
                       allow_agent=False, look_for_keys=False)
        for _i, _comm in enumerate(command_ssh_list, 1):
            _, stdout, _ = client.exec_command(_comm)
...
# Ejecutar el programa en C y capturar la salida
def get_ip_lte_ssh_old(ip_remote : str, fullpath_script : str, usuario : str = "admin", contraseña : str = "HCG", delay_max : int = 15) -> dict:
    json_output = {}
    try:
...
# Ejecutar el programa en C de forma asincrónica y capturar la salida
async def get_ip_lte_ssh(ip_remote: str, fullpath_script: str, usuario: str = "admin", contraseña: str = "HCG", delay_max: int = 15) -> dict:
    json_output = {}
    try:
        # Crear el proceso de forma asincrónica
        ...
        try:
            stdout, stderr = await asyncio.wait_for(proceso.communicate(), timeout=delay_max)
        except asyncio.TimeoutError:
            print(f"Tiempo de espera agotado para {ip_remote}")
            return json_output  # Devuelve un dict vacío si hay timeout
        
        if stderr:
            print(f"Error en {ip_remote}: {stderr.decode().strip()}")

        # Procesar la salida JSON
        salida_json = stdout.decode().strip()
        json_output.update(procesar_salida(salida_json))
