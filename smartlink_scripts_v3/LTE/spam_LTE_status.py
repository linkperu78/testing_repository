...
ssh_script_c    = os.path.join("/usr/smartlink/LTE", script_ssh_name)
...
MAX_TIEMPO_ESTADO       = 5 * INTERVALO_TIEMPO
...
async def hacer_peticion(ip):
    valor_json_ip       = get_ip_lte_ssh(ip, 
                                         ...
                                         delay_max  = MAX_TIEMPO_ESTADO
                                         )

    ##  - - - - - - - DELAY FALSO ------ 
    #tiempo_respuesta = random.randint(0, 20)
    #print(f" -> -> {ip} delay = {tiempo_respuesta}s")
    ...
    while True:
        ...
        for ip, values in dict_ip_status.items():
            ...
            if elapse_time < INTERVALO_TIEMPO + TOLERANCIA_INTERVALO:
                values[0] = ESTADOS[1]
            elif elapse_time > MAX_TIEMPO_ESTADO:
                values[0] = ESTADOS[3]
            else:
                values[0] = ESTADOS[2]
...
async def actualizar_lista_ips() -> None:
    ...
    while True:
        ...
        for ip in lista_ips:
            dict_ip_status.setdefault(ip, [ESTADOS[3], after_time - MAX_TIEMPO_ESTADO])
        ...
        await asyncio.sleep(INTERVALO_INVENTARIO - after_time + current_time)
